...
void motors_config(_motor_t *motors)
{
	...
	for(i = 0; i < sizeof(_motors) / sizeof(_motors[0]); i++) {
		timer_set_channel_compare(motors[i].timer, motors[i].channel,
				0, NULL, NULL);
		timer_activate_channel_output(motors[i].timer, motors[i].channel,
				TIMER_OUTPUT_MODE_PWM1);
	}
}
...
void ppm_update(_motor_t motor, float ratio)
{
	...
	timer_update_channel_compare(motor.timer, motor.channel,
			(uint16_t)((float)_span * ratio + (float)_value_0));
}
