...
void lis302dl_init()
{
	...
	ctrl = (uint8_t) (LIS302DL_CTRL1_PD | LIS302DL_CTRL1_XEN | \
...
void lis302dl_write(uint8_t* data, uint8_t reg_addr, uint16_t length)
{
	/* Configure the MS bit:
       - When 0, the address will remain unchanged in multiple r/w commands.
       - When 1, the address will be auto incremented in multiple r/w commands.
	 */
	if (length > 0x01)
